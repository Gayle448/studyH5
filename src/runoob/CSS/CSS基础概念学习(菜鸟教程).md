...
学习CSS(Cascading Style Sheets)笔记:

CSS规则由两个主要部分构成: 选择器 (通常是需要改变的html元素),以及一条或多条声明 (每条声明由一个样式属性和一个值组成):
h1 {color:blue;font-size:12px}
CSS声明总以;结束,总以大括号{}括起来;
/**/ : CSS注释
...
* .b a {class名为a的标签,里面包含的所有名为b的标签}
* a b c {后代选择器/包含选择器:a 下级元素 b 下级元素 c 满足这里面定义的样式,非严格要求3层,可插入其他元素多层只要结构顺序如此}
* a>b {子选择器,类似于后代选择器,但是不能隔代,更严格,要挨着不能插入其他元素}
* [target]{对应属性 的标签满足 这里定义的样式}
* a~b {兄弟选择器,a b同级别元素,a后面的b元素起作用,不需要紧跟着}
* a+b {兄弟相邻选择器,a b 同级别元素,a后面紧跟着的b元素起作用}
* a:link{所有未访问的链接 满足这里定义的样式}
* p:after{等等 p 标签相关满足条件 样式}
* *{color:red} 通用选择器,匹配所有元素标签.
* > 综合来看两个元素除了逗号,都是标签包含关系.
* #a,b {一个id为a或者一个标签是b的元素的样式}
* #a b {一个id为a下级标签是b的样式}
* #a:b {一个id为a的伪类}
* #a.b {一个id为a的下级标签class是b的元素的样式}

优先级: 标签内 > id选择器 > 类选择器 > 标签选择器
